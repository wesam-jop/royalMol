  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (!catRes.ok) throw new Error('فشل جلب بيانات الفئة');
        if (!prodRes.ok) throw new Error('فشل جلب المنتجات');
        const catData = await catRes.json();
        const prodData = await prodRes.json();
        setCategory(catData.data || catData.category || null);
                    ...
                    <div className="py-4">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-6">
                            <div className="bg-blue-50 p-3 rounded-xl">
                                ...
                                <div className="text-gray-700">{product.category?.name || category?.name || "غير محدد"}</div>
                            </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 p-4 md:p-8" dir="rtl">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          {category?.image && (
            <img src={category.image} alt={category.name} className="mt-4 w-full max-h-64 object-cover rounded-xl" />
          )}
        </div>
